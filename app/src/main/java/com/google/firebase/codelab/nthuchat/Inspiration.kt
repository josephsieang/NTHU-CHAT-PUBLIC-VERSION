...
private const val TAG = "Inspiration"
...
class Inspiration : Fragment() {
    ...
    private var inspirationRecyclerView: RecyclerView? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        activity!!.title = getString(R.string.inspiration)
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val contentView = inflater.inflate(R.layout.fragment_inspiration, container, false)

        Log.d(TAG, "onCreateView()")

        mRefreshLayout = contentView.findViewById(R.id.swipyrefreshlayout) as SwipyRefreshLayout
        mProgressBar = contentView.findViewById<View>(R.id.progressBarInspiration) as ProgressBar
...
//        mapImage = contentView.findViewById(R.id.map_image) as CircleImageView
        ...
        inspirationRecyclerView = contentView.findViewById<View>(R.id.InspirationRecyclerView) as RecyclerView
        mFirebaseDatabaseRef = FirebaseDatabase.getInstance().reference
        val options = FirebaseRecyclerOptions.Builder<ActivityMessages>()
                .setQuery( mFirebaseDatabaseRef!!.child("activity").child("innovation"), ActivityMessages::class.java)
                .setLifecycleOwner(this)
                .build()


        mFirebaseAdapter = object : FirebaseRecyclerAdapter<ActivityMessages, Inspiration.MessageViewHolder>(options) {

            override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Inspiration.MessageViewHolder {
            ...
            override fun onBindViewHolder(viewHolder: Inspiration.MessageViewHolder, position: Int, activityMessage: ActivityMessages) {
...
//                    viewHolder.bindView(activityMessageList.get(position))
                    ...
                    if(activityMessageList[position].creator == mUid) {
                        viewHolder.itemView.setOnClickListener {
                            ...
                            builder.setItems(options, DialogInterface.OnClickListener { dialogInterface, i ->
                                ...
                                if (i == 0) {
                                    //show info
                                    val intent = Intent(activity, InfoActivityInspiration::class.java)
                                    intent.putExtra("activityUid", activityUidList[position])
                                    activity!!.startActivity(intent)
                                } else if (i == 1){
                                    //delete
                                    mFirebaseDatabaseRef!!.child("activity").child("innovation").addListenerForSingleValueEvent(object: ValueEventListener {
                                            ...
                                            for(childSnapshot in dataSnapshot.children) {
                                                if(childSnapshot.key == viewHolder.activityUid.text) {
                                                    ...
                                                    builder.setPositiveButton(getString(R.string.confirm)) { dialog, which ->
                                                        ...
                                                        mFirebaseDatabaseRef!!.child("activity").child("innovation").child(viewHolder.activityUid.text.toString()).removeValue()
        ...
        inspirationRecyclerView!!.layoutManager = mLinearLayoutManager
        inspirationRecyclerView!!.adapter = mFirebaseAdapter
        inspirationRecyclerView!!.itemAnimator!!.changeDuration = 0 //fix blinking when load fragment
        inspirationRecyclerView!!.itemAnimator!!.moveDuration = 0 //fix blinking when load fragment
        inspirationRecyclerView!!.itemAnimator!!.removeDuration = 0 //fix blinking when load fragment
        inspirationRecyclerView!!.itemAnimator!!.addDuration = 0 //fix blinking when load fragment
    ...
    private fun loadMessages() {
        val messagesRef = mFirebaseDatabaseRef!!.child("activity").child("innovation")
...
//                mFirebaseAdapter.notifyItemRangeChanged(init, activityMessageList.size)
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        when(item!!.itemId) {
            R.id.mapBtn -> {
                val myIintent: Intent = Intent(activity, showInspirationActivityMap::class.java)
