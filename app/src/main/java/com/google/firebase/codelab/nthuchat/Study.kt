...
class Study : Fragment() {
    ...
    private var studyRecyclerView: RecyclerView? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        activity!!.title = getString(R.string.study)
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val contentView = inflater.inflate(R.layout.fragment_study, container, false)

        Log.d(TAG, "onCreateView()")

        mRefreshLayout = contentView.findViewById(R.id.swipyrefreshlayout) as SwipyRefreshLayout
        mProgressBar = contentView.findViewById<View>(R.id.progressBarStudy) as ProgressBar
...
//        mapImage = contentView.findViewById(R.id.map_image) as CircleImageView
        ...
        studyRecyclerView = contentView.findViewById<View>(R.id.studyRecyclerView) as RecyclerView
        mFirebaseDatabaseRef = FirebaseDatabase.getInstance().reference
        val options = FirebaseRecyclerOptions.Builder<ActivityMessages>()
                .setQuery( mFirebaseDatabaseRef!!.child("activity").child("learning"), ActivityMessages::class.java)
                .setLifecycleOwner(this)
                .build()


        mFirebaseAdapter = object : FirebaseRecyclerAdapter<ActivityMessages, Study.MessageViewHolder>(options) {

            override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Study.MessageViewHolder {
            ...
            override fun onBindViewHolder(viewHolder: Study.MessageViewHolder, position: Int, activityMessage: ActivityMessages) {
...
//                    viewHolder.bindView(activityMessageList.get(position))
                    ...
                    if(activityMessageList[position].creator == mUid) {
                        viewHolder.itemView.setOnClickListener {
                            ...
                            builder.setItems(options, DialogInterface.OnClickListener { dialogInterface, i ->
                                ...
                                if (i == 0) {
                                    //show info
                                    val intent = Intent(activity, InfoActivityStudy::class.java)
                                    intent.putExtra("activityUid", activityUidList[position])
                                    activity!!.startActivity(intent)
                                } else if (i == 1){
                                    //delete
                                    mFirebaseDatabaseRef!!.child("activity").child("learning").addListenerForSingleValueEvent(object: ValueEventListener {
                                            ...
                                            for(childSnapshot in dataSnapshot.children) {
                                                if(childSnapshot.key == viewHolder.activityUid.text) {
                                                    ...
                                                    builder.setPositiveButton(getString(R.string.confirm)) { dialog, which ->
                                                        ...
                                                        mFirebaseDatabaseRef!!.child("activity").child("learning").child(viewHolder.activityUid.text.toString()).removeValue()
        ...
        studyRecyclerView!!.layoutManager = mLinearLayoutManager
        studyRecyclerView!!.adapter = mFirebaseAdapter
        studyRecyclerView!!.itemAnimator!!.changeDuration = 0 //fix blinking when load fragment
        studyRecyclerView!!.itemAnimator!!.moveDuration = 0 //fix blinking when load fragment
        studyRecyclerView!!.itemAnimator!!.removeDuration = 0 //fix blinking when load fragment
        studyRecyclerView!!.itemAnimator!!.addDuration = 0 //fix blinking when load fragment
    ...
    private fun loadMessages() {
        val messagesRef = mFirebaseDatabaseRef!!.child("activity").child("learning")
...
//                mFirebaseAdapter.notifyItemRangeChanged(init, activityMessageList.size)
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        when(item!!.itemId) {
            R.id.mapBtn -> {
                val myIintent: Intent = Intent(activity, showStudyActivityMap::class.java)
