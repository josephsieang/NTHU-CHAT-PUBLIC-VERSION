...
private const val TAG = "showStudyMap"

class showStudyActivityMap : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun showActionBar() {
        ...
        activityTitle!!.text = getString(R.string.studyMap)
    }
...
//        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(nthu, 16.5F))
        ...
        mFirebaseDatabaseRef = FirebaseDatabase.getInstance().reference.child("activity").child("learning")
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                for(childSnapshot in dataSnapshot.children) {
                    ...
                    if(dateFromString > Calendar.getInstance().time) { //load activity that not expired
                        Log.d(com.google.firebase.codelab.nthuchat.TAG, "activity title: ${activityMessage!!.title}")
                        ...
                        mMap.setOnInfoWindowClickListener {
                            val intent = Intent(baseContext, InfoActivityStudy::class.java)
                            val activityUid = it.tag
                            Log.d(com.google.firebase.codelab.nthuchat.TAG, "activityUid: $activityUid")
